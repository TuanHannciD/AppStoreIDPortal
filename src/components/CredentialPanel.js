...
export default function CredentialPanel({ app }) {
  ...
  const [lastRevealedAt, setLastRevealedAt] = useState(credentials?.lastRevealedAt || null);

  const accountMasked = useMemo(() => maskEmail(credentials?.accountEmail), [credentials?.accountEmail]);
  ...
  async function copy(value, label) {
    ...
    try {
      ...
      pushToast({ title: 'Copied', message: `${label} copied to clipboard.`, intent: 'success' });
    } catch {
      pushToast({ title: 'Copy failed', message: 'Unable to copy to clipboard.', intent: 'danger' });
    }
  }

  const lastRevealedLabel = lastRevealedAt ? formatDate(new Date(lastRevealedAt)) : 'Never (mock)';
    ...
    <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-card">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-600">Account and password are mock values for UI flow.</p>
        </div>
      ...
      <div className="mt-5 space-y-4">
          ...
          <div className="grid gap-3">
            ...
            <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
              <div className="text-xs text-slate-500">
                Last revealed: <span className="font-medium text-slate-700">{lastRevealedLabel}</span>
              </div>
              <button
                type="button"
        ...
        {Array.isArray(credentials?.notes) && credentials.notes.length > 0 ? (
          <div className="rounded-xl border border-slate-200 bg-white p-4">
            <div className="text-xs font-semibold uppercase tracking-wide text-slate-500">Important</div>
        ...
        <div className="rounded-xl border border-amber-200 bg-amber-50 p-4">
          <div className="text-sm font-semibold text-amber-900">Security reminder</div>
          <p className="mt-1 text-sm text-amber-900/80">
            Credentials are sensitive. Do not share, do not change account settings, and sign out after usage.
          </p>
        </div>
      </div>
    </div>
